...
namespace battleShip_cmd
{
    ...
    public class Program
    {
        ...
        {
            int randint2 = random1.Next(0, board.Length - 1); // issue with object reference
        ...
        static void Main()
        {
            ...
            else if (GameMode == 0)
            {
                ...
                for (int n = NumbersOfSize; n == 1; n--) // i hope i set up it well //variable scope issue : i want use (1)
                {
                    ...
                    if (((Math.Pow(BoardSize, 2) - total) - number * n) < 0)
                    {
                        ...
                        int number = Int32.Parse(Console.ReadLine()); // ask to write a correct input // variable scope issue  : i want use (2)
                    }

                    else
                    {
                        Boat.Add(number); //for the boat generator i should use .append() or .Add() ??
                        int total = +number * (n); // Variable scope issue (x)

                        Console.WriteLine(" ");


                        Console.WriteLine("number of rounds (more than " + total.ToString() + " and less than " + (Math.Pow(BoardSize, 2)).ToString() + "): "); //(x)
                    ...
                    {
                        Console.WriteLine("your number of rounds must be beetween " + total.ToString() + " and " + (Math.Pow(BoardSize, 2)).ToString() + " : "); //(x)
            ...
            {  // dont remenber where this for end ...............
                ...
                {
                    Console.WriteLine("Turn " + (turn + 1).ToString + " :"); // what's wrong here ?
                    Console.WriteLine(" ");
                ...
                if (guess_row - 1 == ship_row && guess_col - 1 == ship_col) // variable scope issue : i want use (3) and (4)
                {
                    Console.WriteLine("Congratulations! You sunk my battleship!");
                    board[guess_row - 1][guess_col - 1] = "V"; // the [[]; []] // variable scope issue : i want use (3) and (4)
                    print_board(board); //why i can't call the method ?
                    break;
                }
                ...
                else
                {
                    ...
                    if ((guess_row - 1 < 0 || guess_row - 1 > BoardSize - 1) || (guess_col - 1 < 0 || guess_col - 1 > BoardSize - 1)) //variable scope issue : i want use (3) and (4)
                    {
                    ...
                    else if ((board[guess_row - 1][guess_col - 1] == "X")) //variable scope issue : i want use (3) and (4)
                    {
